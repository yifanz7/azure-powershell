...
namespace Microsoft.WindowsAzure.Commands.Storage.Common
{
    ...
    internal static class StorageExtensions
    {
        ...
        internal static Uri GenerateUriWithCredentialsForTrack2Blob(this BlobBaseClient blob, AzureStorageContext context)
        {
            if (blob == null)
            {
                throw new ArgumentNullException("blob");
            ...
            {
                return blob.GenerateSasUri(BlobSasPermissions.Read, DateTimeOffset.UtcNow.Add(sasLifeTime));
            }
            else
            {
                if (blob.Uri.Query != null && blob.Uri.Query.Contains("sig=")) // SAS 
                {
                    return blob.Uri;
                }
                if (context != null && context.StorageAccount != null && context.StorageAccount.Credentials != null && context.StorageAccount.Credentials.IsToken) // OAuth
                {
                    ...
                    string sasToken = sasBuilder.ToSasQueryParameters(userDelegationKey, context.StorageAccountName).ToString();
